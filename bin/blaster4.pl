...
~/repo/blaster4.pl blastlist-archaea
...
for ($i=0;$i<scalar(@taxa1);$i++) {
  ...
  if ($printcount == 0) {
    ...
    if ($forced ==1) {
      print $fhfile1 "rm $prok/$taxon1/$taxon1.$taxon2.hits\n";
      print $fhfile1 "/home/rec3141/repo/signifier.sh $taxon1 $taxon2\n";
      print $fhfile1 "rm $prok/$taxon2/$taxon2.$taxon1.hits\n";
      print $fhfile1 "/home/rec3141/repo/signifier.sh $taxon2 $taxon1\n";
      $printcount++;
      $printcount++;
      print "n"; $newjobs++;
    } else {
