...
my %genehash; #   %genehash{'NC_111000'} = 'NC_111000'
# tie %genehash, 'Tie::RefHash';
# a hash of which genes are in each cluster
my %clusterhash; #   %clusterhash{'NC_111123'}{'NC_111000'} = undef, %clusterhash{'NC_111123'}{'NC_111111'} = undef
# tie %clusterhash, 'Tie::RefHash::Nestable';
...
foreach my $subject (@query_groupids) {
	...
	%genehash = %clusterhash = %hoh1 = %hoh2 = %redo = ();
	$missing = 0;
	...
	# Finish Clustering
	$starttime = time;
	print "found " . scalar(keys %clusterhash) . " clusters\n" if $debug;
...
sub checkOptions {
	...
	my ($accgenomes_ref, $genes_ref) = parse_accession_index( {'all_ids' => \%query_all_ids, 'parse_locus' => 1, 'accdirs' => \@accdirs } );
	my %accgenomes = %{$accgenomes_ref};

	foreach (keys %{ $genes_ref }) { $allgenelist{$_} = undef };
	undef($genes_ref);
		...
		my @list = @{ shift(@_) };
		 HITS: foreach my $hit (@list) {
		  ...
		  # find if proteins not in accession_list						
			unless (  exists($allgenelist{$hit_id}) &&
					  exists($allgenelist{$query_id}) ) {
...
sub procMCL {
  ...
  my $hit = shift;
  my $eval = shift;
#v6   $genelist{$query} = undef;
#   $genelist{$hit} = undef;
  delete($genelist{$query});
  delete($genelist{$hit});
...
#v6    procMCL($k1,$k1,0) if defined($genelist{$k1});
   procMCL($k1,$k1,0);
 }
 undef %genelist;
...
sub reconcile {
	my %reco = ();
	my %seen = ();
#v5 	tie %reco, 'Tie::RefHash::Nestable';
	%hoh1 = %genehash = %clusterhash = %genelist = ();
#v5 	tie %hoh1, 'Tie::RefHash::Nestable';
...
# # #	print "$cluster_outfile.cluster_list.tmp exists\n" and exit 0 if ( -e "$cluster_outfile.cluster_list.tmp" );
	...
	$starttime = time;
	print "genome\t\%reco\t\%hoh1pre\t\%hoh1post\t\%clusterhash\t\%genehash\ttime\n" if $debug;
	foreach my $subject (@query_groupids) {

		print "$subject\t" if $debug;
		my $infile = "$output_dir/$query_taxid/$query_taxid.$subject.$ecutoff\_$mlf\_$mlf_type\_".lc($recip_flag)."\_$clusttype_flag";
		$infile .= "\_$inflation" if $mcl;

		my @clusters = read_file("$infile.cluster_list");

		foreach my $cluster (@clusters) {
			chomp $cluster;
			next unless $cluster;
			my @prots = split(/\s+/,$cluster);
			my $key = shift(@prots);
			foreach (@prots) {
				if ( exists($reco{$_}{$key}) ) {
					$hoh1{$_}{$key} = 2;
					delete($reco{$_}{$key});
					delete($reco{$_}) unless keys %{$reco{$_}};
				} else {
					$reco{$key}{$_} = undef;
				}
			}
		...
		doCluster();
 		print scalar(keys %hoh1),"\t",scalar(keys %clusterhash),"\t",scalar(keys %genehash),"\t",(time - $starttime),"\n" if $debug;

	}

	$subject_taxid = $query_taxid;
	@cluster_lists = ();
	outputClusters() if $output_list;
	makeHistogram() if $output_hist;
...
# 	$genehash{$k1} = $k1;
# 	$genehash{$k2} = $k1;
# 	$clusterhash{ $k1 }{ $k1 } = undef;
# 	$clusterhash{ $k1 }{ $k2 } = undef;
# }
# 
# add key2 to cluster of key1
# sub addtoCluster {
#   my $k1 = shift;
#   my $k2 = shift;
#   $genehash{$k2} = $genehash{$k1};
#    print "add: $$k1,${$genehash{$k1}} | $$k2,${$genehash{$k2}}\n";
#   $clusterhash{ $genehash{$k1} }{ $k2 } = undef;
# }
# 
...
#   my $k1 = shift;
#   my $k2 = shift;
#    print "mer: $$k1,${$genehash{$k1}} | $$k2,${$genehash{$k2}}\n";
#   
#   my $dead = $genehash{$k2};
#   foreach my $k3 (keys %{ $clusterhash{ $genehash{$k2} }}) {
#    	print "mov: $$k1,${$genehash{$k1}} | $$k2,${$genehash{$k2}} | $$k3,${$genehash{$k3}}\n";
# 	$genehash{$k3} = $genehash{$k1};
# 	$clusterhash{ $genehash{$k1} }{ $k3 } = undef;
#   }
#   #then delete second cluster
#   delete( $clusterhash{$dead} );
# }


sub doCluster {
  # $hoh1{$ref_to_NP_111123}{$ref_to_NP_000123} = 2
  # $genehash{$ref_to_NP_111123} = $ref_to_NP_000123;
  # $clusterhash{$ref_to_NP_000123}{$ref_to_NP_111123} = undef
  ...
  foreach my $k1 ( keys %hoh1 ) {
    foreach my $k2 ( keys %{$hoh1{$k1}} ) {
	  ...
	  my $gd1 = defined($genehash{$k1}); #can change within my $k2
	  my $gd2 = defined($genehash{$k2});
...
# 		  mergeClusters($k1,$k2);
		  ...
		  if ($recip_flag ne 'rbh') {
		  	# addtoCluster($k2,$k1);
			$genehash{$k1} = $genehash{$k2};
			$clusterhash{ $genehash{$k2} }{ $k1 } = undef;		
		  } else {}
	  }
#v5       $genelist{$$k2} = undef if defined($genehash{$k2});
#v6       $genelist{$k2} = undef if defined($genehash{$k2});
      delete($genelist{$k2}) if defined($genehash{$k2});
      delete($hoh1{$k1}{$k2});      
    }
#v5     $genelist{$$k1} = undef if defined($genehash{$k1});
#     $genelist{$k1} = undef if defined($genehash{$k1});
    delete($genelist{$k1}) if defined($genehash{$k1});
    delete($hoh1{$k1}) unless scalar(keys %{$hoh1{$k1}}); #?? double check
  }
return 1;
}
...
# so later we can sort by values and choose the best 'seed' protein
 ...
 foreach my $k1 ( keys %hoh2 ) {
  foreach my $k2 ( sort { ($hoh2{$k1}{$a} || 1) <=> ($hoh2{$k1}{$b} || 1) } keys %{$hoh2{$k1}} ) {
    ...
    if ($debug > 1) {
    	print "k1\t$k1\tk2\t$k2\tgc1\t$genehash{$k1}\tgc2\t$genehash{$k2}\n" unless (defined($k1) && defined($k2) && defined($genehash{$k1}) && defined($genehash{$k2}));
	}
    last;
  }
#v6   ($genelist{$k1} = undef);
  delete($genelist{$k1});
 }
...
# 	print Dumper %genehash;
	...
	foreach my $key (keys %clusterhash) {
	  if ($recip_flag eq 'rbh') {
# 		  my @deref = map { $_ ne $key ? $allgenelist{${$_}} : undef } keys %{ $clusterhash{$key} };
# 		  push(@cluster_lists, [ $$key, sort grep {defined} @deref ]);      
		  my @deref = map { $_ ne $key ? $_ : undef } keys %{ $clusterhash{$key} };
		  push(@cluster_lists, [ $key, sort grep {defined} @deref ]);      
      } else {
# 		  my @deref = map { $allgenelist{${$_}} } keys %{ $clusterhash{$key} };
# 		  push(@cluster_lists, [ sort @deref ]);
 		  push(@cluster_lists, [ sort keys %{ $clusterhash{$key} } ]);
      }
	}
	undef %clusterhash;

	#first sort the rows of the cluster list by order of genes
 	@cluster_lists = sort {"@$a" cmp "@$b"} @cluster_lists;

	#then sort the rows of the cluster list by number of genes in the cluster
	@cluster_lists = sort {@{$b} <=> @{$a}} @cluster_lists;

	# add the unclustered genes/singletons to @cluster_lists
	if (lc($recip_flag) ne 'rbh') {
	  foreach my $k1 ( sort keys %genelist ) {
...
=item B<example:>
	...
	distcluster.pl -t taxalist-Bacillus -q BL_110001 -e 1e-20 -f 0.8 -r seed
...
520 genomes were semi-clustered in 5 hours on 36 CPUs using 1.2 Gb of memory each
final clustering ("reconciliation") took 30 minutes on a single CPU using 5 Gb of memory

=head1 CHANGES

=item B<2/04/2013:> significantly increase speed of reconciliation
